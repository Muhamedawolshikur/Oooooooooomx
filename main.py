...
def control_buttons(uid):
    ...
    markup.row(types.KeyboardButton("📊 Live Logs"), types.KeyboardButton("⏹ Stop All Bots"))
    markup.row(types.KeyboardButton("🚀 Server Status"), types.KeyboardButton("🆘 Support"))
    if uid == OWNER_ID:
...
def keyboard_handler(m):
    ...
    if text == "🌏 Upload Bot":
        ...
        if not user_logs or not any(user_logs.values()): 
            return bot.reply_to(m, "❌ There are no active bots streaming output right now.")
        for file_path, lines in user_logs.items():
            if lines:
                last_lines = "\n".join(lines[-15:])
                bot.send_message(uid, f"📊 *Live Terminal Output:* `{os.path.basename(file_path)}`\n```\n{last_lines}\n```", parse_mode="Markdown")

    elif text == "⏹ Stop All Bots":
        if uid in active_scripts and active_scripts[uid]:
            for p in list(active_scripts[uid].values()): 
                p.kill()
            active_scripts[uid] = {}
            bot.reply_to(m, "⏹ Global Kill Signal Executed. All your processes have been terminated.")
        else:
            bot.reply_to(m, "❌ You have no background scripts currently running.")
        ...
        status_text = (
            "⚙️ *CORE SYSTEM METRICS*\n\n"
            f"👥 *Global Registered Users:* `{total_users}`\n"
            f"🟢 *Active Virtual Terminals:* `{running_scripts}`\n"
            f"⏱️ *Host Infrastructure Uptime:* `{uptime}`"
        )
        ...
        help_text = (
            ...
            "4️⃣ Inspect continuous stdout errors via the *'Live Logs'* protocol."
        )
